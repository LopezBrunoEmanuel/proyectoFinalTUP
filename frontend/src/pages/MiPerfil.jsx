...
const MiPerfil = () => {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      const msg =
        backendMsg ||
        (status === 401
          ? "Seson invalida o expirada. Volve a iniciar sesion."
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    if (nuevaPassword !== confirmarNuevaPassword) {
      setUiMsg({
        type: "danger",
        text: "La contraseña debe contener entre 8 y 30 caracteres",
      });
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
    }
    ...
    try {
      ...
      if (!data?.success) {
        setUiMsg({
          type: "danger",
          text: "Respuesta invalida del servidoral actualizar la contraseña",
        });
        window.scrollTo({ top: 0, behavior: "smooth" });
        return;
      }
        ...
        <div className="mi-perfil-header">
          ...
          <div className="mi-perfil-status">
            ...
            {user.activo ? (
              ...
              <span className="status-pill status-bad text-danger">
                Cuenta inactiva
              </span>
            )}
          </div>
